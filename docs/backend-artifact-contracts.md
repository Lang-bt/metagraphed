...
- `/metagraph/subnets/{netuid}/economics/history.json`: schema for a subnet's per-day economics history (alpha price, emission share, total stake, registration cost), served live from the `economics_history` D1 rollup at `GET /api/v1/subnets/{netuid}/economics/history` (no static file).
...
- `/api/v1/subnets/{netuid}/economics/history`: fetch a subnet's per-day economics history (alpha price, emission share, total stake, registration cost) over a `?window=7d|30d|90d|1y|all` window (live from the `economics_history` D1 rollup, snapshotted daily off the live economics tier).
